...
void creatertpkt(t_rtpkt *initrtpkt, int srcid, int destid, int mincosts[])
{
    int i;
...
extern void rtupdate0(t_rtpkt *);
extern void rtupdate1(t_rtpkt *);
extern void rtupdate2(t_rtpkt *);
extern void rtupdate3(t_rtpkt *);
    ...
    while (1) {
        ...
        if (TRACE>1) {
            ...
            if (eventptr->evtype == FROM_LAYER2 ) {
                ...
                printf(" contents: %3d %3d %3d %3d\n", eventptr->rtpktptr->mincost[0], eventptr->rtpktptr->mincost[1], eventptr->rtpktptr->mincost[2], eventptr->rtpktptr->mincost[3]);
            }
        }
...
void tolayer2(struct rtpkt packet)
{
    ...
    if (TRACE>2)  {
        printf("    TOLAYER2: source: %d, dest: %d\n              costs:", mypktptr->sourceid, mypktptr->destid);
        for (i=0; i<4; i++)
