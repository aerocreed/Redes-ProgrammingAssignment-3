...
void rtupdate1(t_rtpkt *rcvdpkt)
{
    ...
    for(int i = 0; i < 4; ++i) {
        ...
        if(rcvdpkt->mincost[i] < dt1.costs[j][i])
            dt1.costs[j][i] = rcvdpkt->mincost[i];
        
        //se a distancia (X --> i) ja conhecida
        //eh maior que a distancia (X --> j) + (distancia j --> i) ....
        //... encontramos um menor caminho =D
        if(pkt1->mincost[i] > dt1.costs[1][j] + rcvdpkt->mincost[i]) {
            pkt1->mincost[i] = dt1.costs[1][j] + rcvdpkt->mincost[i]; //NOVO MENOR CAMINHO ATRAVES DE X --> j --> i
